...
The Bluetooth reference device can be of different types, such as BES device
(single BES Bluetooth board), or TWS device (a pair of BES Bluetooth boards).
The Mobly controller of each device type is defined in its own module, derived
from the `BluetoothReferenceDeviceBase` class.
...
from android_beat.bluetooth.platforms.bluetooth import bes_device
from android_beat.bluetooth.platforms.bluetooth import bluetooth_reference_device_base
...
_SUPPORTED_DEVICE_CLASSES = immutabledict.immutabledict({
    ...
    tws_device.MOBLY_CONTROLLER_CONFIG_NAME: tws_device,
})
...
def create(
  ...
  for config in configs:
    ...
    if config['controller_name'] not in _SUPPORTED_DEVICE_CLASSES:
    ...
    try:
      devices.extend(
          _SUPPORTED_DEVICE_CLASSES[config['controller_name']].create([config])
      )
