...
class LogcatEventSubscriber(LogcatSubscriber):
  ...
  def __init__(self, publisher, pattern='.*', tag='*', level='V'):
    ...
    self._tag = self._process_tag(tag)
  ...
  def _process_tag(self, tag: str) -> Union[str, re.Pattern[str]]:
    if tag == '*':
      return tag
    else:
  ...
  def handle(self, data):
    ...
    if data.tag is None:
      return
    if isinstance(self._tag, str):  #  wildcard case
      if not fnmatch.fnmatchcase(data.tag, self._tag):
        return
    else:  # regex case
      if not self._tag.match(data.tag):
        return
