...
import call_audio_state
import telecom_base_test
from utils import media_utils as telecom_media_utils
...
class VoipCallWithLeaHeadsetTest(telecom_base_test.TelecomBaseTest):
  """Voice over IP call test with LEA headset."""

  _BLUETOOTH_MODE = telecom_base_test.TelecomBaseState.LEA
  _DEVICE_NUM = telecom_base_test.TelecomBaseDeviceNum.SINGLE_DEVICE
  _IS_VOIP_TEST = True

  def test_voip_outgoing_call_through_lea_headset(self):
    """Test outgoing VoIP call through LEA headset.
    ...
    3. The call audio is routed to LEA headset.
  ...
  def test_voip_outgoing_call_through_lea_headset_during_media_playback(self):
    """Test outgoing VoIP call through LEA headset during media playback.
    ...
    Pass Criteria:
      1. Local media is played through the LEA headset before the call, verified
         by LEA state and BES Audio Recorder.
      2. The call can be established and local media is paused, verified by
         isMusicActive and BES Audio Recorder.
      3. Call audio is routed through the LEA headset, verified via BES Audio
         Recorder.
      4. The call state is STATE_DISCONNECTED via the onStateChanged callback
         after call is terminated.
      5. Local media is resumed after the call, verified by LEA state,
         isMusicActive and BES Audio Recorder.
    """
    ...
    telecom_media_utils.wait_for_lea_state(
        self.ad, self.bt_address_primary, expect_lea_active=True
    )
    # Registers a self-managed account for VoIP call.
    self.ad.tele.registerTransactionalPhoneAccount()
    asserts.assert_true(
  ...
  def test_incoming_call_through_lea_headset(self):
    ...
    Pass Criteria:
      1. The call count for self-managed calls is 1 during the call.
      2. The call is correctly identified as incoming.
      3. Call audio is played through LEA headset.
      4. The call state becomes STATE_ACTIVE after call is answered.
      5. Audio is played through LEA headset.
      6. The call state becomes STATE_DISCONNECTED after call is terminated.
    """
    # Registers a self-managed account for VoIP call.
    self.ad.tele.registerTransactionalPhoneAccount()
  ...
  def test_incoming_call_through_lea_headset_during_media_playback(self):
    """Test incoming VoIP call through LEA headset during media playback.
    ...
    Pass Criteria:
      1. Local media is played through the LEA headset before the call,
         verified by LEA state.
      2. The call can be established and local media is paused, verified by
         isMusicActive.
      3. Call audio is routed through the LEA headset.
      4. The call state is STATE_DISCONNECTED via the onStateChanged callback
         after call is terminated.
      5. Local media is resumed after the call, verified by LEA state,
         isMusicActive.
  ...
  def test_mid_call_audio_route_switch_lea_headset_to_speaker(self):
    """Test mid call audio route switch from LEA headset to speaker.

    Objective:
      To verify that the audio route can be switched between the LEA headset
      and the device speaker during an active VoIP call.

    Test Steps:
      1. DUT registers a self-managed account for VoIP call.
      2. DUT initiates an outgoing VoIP call.
      3. DUT verifies the call is active and audio is routed to the LEA headset.
      4. DUT switches the audio route to the device speaker.
      5. DUT switches the audio route back to the LEA headset.
      6. DUT ends the call.

    Pass Criteria:
      1. The call becomes active and audio is initially routed to the LEA
         headset.
      2. The audio route successfully switches to the device speaker.
      3. The audio route successfully switches back to the LEA headset.
    ...
    self.ad.log.info('VoIP call to %s is ended', _TEST_DIAL_NUMBER)
  # function after reboot. Therefore we set it as a private function for now.
  def _test_mid_call_audio_route_switch_lea_headset_power_off_on(self):
    """Test mid call audio route switch from LEA headset to speaker.

    Objective:
      To verify that the audio route can be switched from the LEA headset to
      the device earpiece during an active VoIP call when the LEA headset is
      powered off and back to the LEA headset when it is powered back on.

    Test Steps:
      1. DUT registers a self-managed account for VoIP call.
      2. DUT initiates an outgoing VoIP call.
      3. DUT verifies the call is active and audio is routed to the LEA headset.
      4. Power off the LEA headset.
      5. DUT verifies the audio route successfully switches to the earpiece.
      6. Power on the LEA headset.
      7. DUT verifies the audio route successfully switches back to the LEA
         headset.
      8. DUT ends the call.
    ...
    Pass Criteria:
      1. The call becomes active and audio is initially routed to the LEA
         headset.
      2. The audio route successfully switches to the earpiece after the LEA
         headset is powered off.
      3. The audio route successfully switches back to the LEA headset after
         the LEA headset is powered on.
      4. The call state becomes STATE_DISCONNECTED after the call is terminated.
    """
    # Registers a self-managed account for VoIP call.
    self.ad.tele.registerTransactionalPhoneAccount()
  ...
  def _test_mid_call_audio_route_switch_lea_headset_power_on(self):
    """Test mid call audio route switch from speaker to LEA headset.

    Objective:
      To verify that the audio route can be switched from the device speaker to
      the LEA headset during an active VoIP call when the LEA headset is
      powered on.

    Test Steps:
      1. Power off the LEA headset.
      2. DUT registers a self-managed account for VoIP call.
      3. DUT initiates an outgoing VoIP call.
      4. DUT verifies the call is active and audio is routed to the earpiece.
      5. Powers on the LEA headset.
      6. DUT verifies the audio route successfully switches to the LEA headset.
      7. DUT ends the call.

    Pass Criteria:
      1. The call becomes active and audio is initially routed to the earpiece.
      2. The audio route successfully switches to the LEA headset after the
         LEA headset is powered on.
    ...
    self.ad.log.info('VoIP call to %s is ended', _TEST_DIAL_NUMBER)
  # lea state. Therefore we set it as a private function for now, and will
  # enable it once the call can be ended by call_decline in lea state.
  def _test_reject_incoming_call_through_lea_headset(self):
    """Test reject incoming VoIP call through LEA headset.
    ...
    Pass Criteria:
      ...
      3. The call audio is routed to LEA headset during ringing.
  ...
  def _test_mid_call_audio_route_switch_bluetooth_off_on(self):
    ...
    Objective:
      ...
      off and back to the LEA headset when it is turned back on.
    ...
    # Originates a VoIP call to the test number.
    self.ad.log.info('Originating VoIP call to %s', _TEST_DIAL_NUMBER)
    call_id = self.ad.tele.addTransactionalCall(_TEST_DIAL_NUMBER)
    self.ad.log.info('Call added successfully, call_id: %s', call_id)
    # Verify the call state is STATE_DIALING.
  ...
  def _test_end_ongoing_outgoing_call_through_lea_headset(self):
    ...
    Pass Criteria:
      ...
      2. The call audio is routed to LEA headset during active call.
  ...
  # 'DIALING' state.
  def _test_end_outgoing_call_through_lea_headset(self):
    """Test end outgoing VoIP call through LEA headset.
    ...
    Pass Criteria:
      ...
      2. The call audio is routed to LEA headset during ringing.
      3. The call state becomes STATE_DISCONNECTED after BT device ends
         the call.
    """
    # Registers a self-managed account for VoIP call.
