# android_telecq/utils/telecom_utils.py
# Lint as: python3
...
from android_telecq import telecom_timeout_constants as timeout_constants
from android_telecq.utils import telecom_ui_utils
...
def monitor_in_call(
    ad: android_device.AndroidDevice,
    remote_phone_number: str,
    expected_audio_state: dict[str, int],
    duration: datetime.timedelta = timeout_constants.TIMEOUT_SHORT,
) -> None:
  """Monitors the call state and audio state during a call.

  Test Steps:
    1. Check audio state
    2. Wait for few seconds
    3. Check if call is still connected
    4. Check audio state again

  Args:
    ad: The Android device under test.
    remote_phone_number: The phone number of the remote device.
    expected_audio_state: The expected audio state (e.g. BT, earpiece).
    duration: The duration to wait between checks.
  """
  ad.log.info('Checking initial audio state: %s', expected_audio_state)
  ad.tele.waitForAudioState(expected_audio_state)
  asserts.assert_true(
      ad.tele.telecomGetAudioState(expected_audio_state),
      f'{ad} Audio state is not as expected: {expected_audio_state}',
  )

  ad.log.info('Waiting for %s seconds...', duration.total_seconds())
  time.sleep(duration.total_seconds())

  ad.log.info('Checking if call is still connected...')
  asserts.assert_equal(
      ad.tele.telecomGetCallState(remote_phone_number),
      CallState.STATE_ACTIVE,
      f'{ad} Call is disconnected during the call duration',
  ...
  ad.log.info('Checking audio state again: %s', expected_audio_state)
  asserts.assert_true(
      ad.tele.telecomGetAudioState(expected_audio_state),
      f'{ad} Audio state is not as expected: {expected_audio_state}',
  )
...
def handle_meet_onboarding_permissions(
  ...
  if not search_input.wait.exists(timeout=5000):
    ad.log.warning(
        "Onboarding failed, 'Search contacts' bar never appeared.",
    )
  else:
...
def verify_speaker_bluetooth_endpoint_switching_and_disconnect(
    ad: android_device.AndroidDevice, call_id: str
) -> None:
  """Verifies the speaker and bluetooth endpoint switching and disconnect.

  Args:
    ad: The Android device to verify the endpoint switching and disconnect.
    call_id: The ID of the call to verify the endpoint switching and disconnect.
  """
  # Verify call endpoints switches are working
  # Switch to Speaker
  ad.tele.switchAudioToEndpointType(
      call_id, call_audio_state.CallEndpoint.SPEAKER
  )
  wait_until_or_assert(
      lambda: ad.tele.getTransactionCallEndpointType(call_id)
      == call_audio_state.CallEndpoint.SPEAKER,
      error_msg=f'{ad} Failed to switch endpoint to SPEAKER',
      timeout=WAIT_CALL_CONNECTED_TIMEOUT,
  )

  # Switch to Bluetooth
  ad.tele.switchAudioToEndpointType(
      call_id, call_audio_state.CallEndpoint.BLUETOOTH
  )
  wait_until_or_assert(
      lambda: ad.tele.getTransactionCallEndpointType(call_id)
      == call_audio_state.CallEndpoint.BLUETOOTH,
      error_msg=f'{ad} Failed to switch endpoint to BLUETOOTH',
      timeout=WAIT_CALL_CONNECTED_TIMEOUT,
  )

  # Terminate the call (Action: Hangup)
  ad.tele.hangupTransactionalCall(call_id)
  wait_until_or_assert(
      lambda: ad.tele.getTransactionCallState(call_id)
      == CallState.STATE_DISCONNECTED,
      error_msg=f'{ad} Failed to disconnect the call {call_id}',
      timeout=WAIT_CALL_CONNECTED_TIMEOUT,
  )
